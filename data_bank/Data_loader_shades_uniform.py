...
class Data_loader_shades_uniform(Data_loader):
    """
...
"""
    def __init__(self, arguments):
        super(Data_loader_shades_uniform, self).__init__(arguments)
...
""" % (self.number_of_samples, self.grid_size, self.side_length, self.shade_contrast, self.separate, self.save, self.images, self.labels, self.difference, self.model_number)
    ...
    def _generate_set(self, shuffle= True):
        ...
        for i in range(n):
            ...
            difference_shades = np.random.uniform(e,1)
            shade_random = np.random.randint(2, size = 1)

            if shade_random == 0:
                shade1 = np.random.uniform(0,1-difference_shades)
                shade2 = shade1 + difference_shades

            elif shade_random == 1:
                shade2 = np.random.uniform(0,1-difference_shades)
                shade1 = shade2 + difference_shades
